...
void Play::nearby_event(){
	...
	if (playery + 1 < cavesize && cave[playery+1][playerx].get_event_num() > 1){
		if (cave[playery+1][playerx].get_event_num() != 3 || gold_retrieved == false)
			cout << cave[playery+1][playerx].get_event_percept() << endl;
	}
	if (playerx + 1 < cavesize && cave[playery][playerx+1].get_event_num() > 1){
		if (cave[playery][playerx+1].get_event_num() != 3 || gold_retrieved == false)
		...
		for (int j = 0; j < cavesize; j++){
			...
			if (i == starty && j == startx && debug == "true" && 
				...
				(cave[i][j].get_event_num() < 2 || 
				 cave[i][j].get_event_num() > 5) && debug == "true")
			    cout << "  ";
...
void Play::set_cave(){
	...
	while (placed3 == false){
		...
		if ((randx5 == randx1 && randy5 == randy1) || (randx5 == 
			randx4 && randy5 == randy4))
				placed3 = false;
	...
	for (int i = 0; i < 2; i++){
		...
		while (placed == false){
			...
			if ((randx2 == randx1 && randy2 == randy1) || (randx2 == randx4 && 
				randy2 == randy4) || (randx2 == randx5 && randy2 == randy5))
				placed = false;
	...
	for (int i = 0; i < 2; i++){
		...
		while (placed == false){
			...
			if ((randx3 == randx2 && randy3 == randy2) || (randx3 == randx1
				 && randy3 == randy1) || (randx3 == randx4 && randy3 == randy4)
...
void Play::shoot_arrow(string direction){
	...
	for(int i = 0; i < 3; i++){
        ...
        /* S for South */
		else if (direction == "S" && (arrowy + 1) < cavesize && wumpus_killed 
                == false){
			cout << "Arrow fired South through a room." << endl;
			arrowy = arrowy + 1;				
		}
        /* A for West */
		else if (direction == "A" && (arrowx - 1) >= 0 && wumpus_killed ==
				false){
			cout << "Arrow fired West through a room." << endl;
			arrowx = arrowx - 1;				
		}			
        /* D for East */
		else if (direction == "D" && (arrowx + 1) < cavesize && wumpus_killed 
				== false){
...
bool Play::alive(){
	...
	else if (cave[playery][playerx].get_event_num() == 3 && wumpus_killed == false){
		cout << "You have found the gold! Find and kill the Wumpus now!" << endl;	
		gold_retrieved = true;
		return true;
	}
	else if (cave[playery][playerx].get_event_num() == 3 && wumpus_killed == true){
